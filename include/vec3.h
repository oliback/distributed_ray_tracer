...
public:
	...
	Vec3<T> operator/(T t) { return Vec3<T>(x/t, y/t, z/t); }
	void operator/=(T t) { x /= t; y /= t; z /= t; }
	...
	Vec3<T> reflect(const Vec3 &N) {
		//float d = -(N.dot(*this));
		//return (*this) + (N * 2.0 * d);
		/* TODO: not entirely sure that this is correct... 
		found here http://mathforum.org/kb/message.jspa?messageID=5253727 */
		/*
		Vec3f n;
		float len = N.len();
		n.set(N.x/len, N.y/len, N.z/len);
		Vec3f vpar = (n * (N.dot(*this)));
		return (*this - vpar * 2.0f);
		*/

		/* http://www.ozone3d.net/tutorials/glsl_lighting_phong.php */
		float temp = N.dot(*this) * 2.0f;
		return N * temp - (*this);
	}
};
